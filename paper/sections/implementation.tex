...
\item \textbf{Physics parameters}: Ion gyroradius $\rhoi$, plasma beta $\betai$, \Alfven\ velocity $\vA$, temperature ratio $\taue = T_e/T_i$, moment truncation $\Mmax$
...
The configuration system validates parameter choices against KRMHD validity constraints (small $\betai$, large-scale separation) and numerical stability bounds ($\eta\Delta t < 50$, $\coll\Delta t < 10$ for hyper-dissipation). Example configurations accompanying the code demonstrate standard simulation types: decaying turbulence initialized with Orszag-Tang vortex, forced turbulence at controlled Reynolds numbers, linear wave propagation tests, and anisotropic cascade studies. These examples provide starting points for new users and serve as integration tests for continuous validation.
...
Benchmarks on NVIDIA A100 GPUs demonstrate wall-clock performance for grids ranging from $32^3$ to $256^3$ with $M=16$ moments:

\begin{center}
\begin{tabular}{lrrr}
\hline
Grid & Memory & Time/Poisson & Time/Step \\
\hline
$32^3$   & $\sim$1 MB    & 0.57 ms  & 18 ms   \\
$64^3$   & $\sim$7 MB    & 3.5 ms   & 110 ms  \\
$128^3$  & $\sim$58 MB   & 28 ms    & 0.90 s  \\
$256^3$  & $\sim$460 MB  & 257 ms   & 8.2 s   \\
\hline
\end{tabular}
\end{center}

Memory usage scales as $N^3$ while computation scales as $N^3 \log N$, yielding super-linear cost increases with resolution. A $256^3$ turbulence simulation spanning 100 nonlinear times with typical $\Delta t \sim 0.01 t_{\mathrm{eddy}}$ requires $\sim$10,000 time steps ($\sim$23 hours on A100), feasible for systematic parameter studies. The pure-JAX implementation achieves performance competitive with hand-optimized CUDA codes \citep{Bauer2021,Citrin2024} while remaining portable across hardware platforms without modification.

On Apple Silicon (M1 Pro, M2 Max) using the Metal backend, GANDALF delivers performance within a factor of 2--3 of GPU results for grids up to $128^3$, sufficient for code development, small parameter scans, and educational applications. This portability democratizes access to kinetic plasma turbulence simulations: researchers without GPU cluster access can perform meaningful science on laptop hardware, previously impossible with traditional HPC-focused codes.
...
\paragraph{Energy Diagnostics.} The code computes total energy (sum over Elsasser and Hermite contributions), cross-helicity, and residual energy at each time step, writing time series to HDF5 files. For inviscid, unforced runs, energy conservation provides a stringent numerical accuracy check: deviations exceeding $0.1\%$ over hundreds of nonlinear times indicate insufficient resolution, excessive time steps, or dealiasing failures.
...
\paragraph{Checkpoint Management.} Automatic checkpointing at user-specified intervals enables long simulations to survive hardware failures or scheduler interruptions on shared clusters. Checkpoints store complete state (including RNG seeds for forced runs), allowing bit-exact restarts. Lightweight ``restart files'' containing only essential state variables reduce storage costs for archival purposes.
...
\item Jupyter notebooks demonstrating basic workflows
\end{itemize}
...
The repository employs semantic versioning. Current development focuses on extending the physics model (finite Larmor radius corrections, electromagnetic temperature variations) and improving performance (better FFT libraries, multi-GPU parallelization via \texttt{pmap}). Community contributions addressing bugs, documentation improvements, or new features follow standard GitHub pull-request workflows.
...
\paragraph{GX and GPU Codes.} GX \citep{Mandell2024} represents modern GPU-native gyrokinetic codes achieving exceptional performance through hand-optimized CUDA kernels. GX targets stellarator and tokamak applications where geometric complexity demands specialized optimization. GANDALF's JAX implementation sacrifices peak performance ($\sim$2--3$\times$ slower than optimized CUDA) for hardware portability and development velocity. The performance gap narrows for moderate-scale turbulence problems ($128^3$ grids) where GANDALF's pure-JAX approach proves competitive, while extreme-scale runs ($512^3$ and larger) favor specialized codes like GX.
...
\paragraph{Design Philosophy.} GANDALF's core differentiator lies in accessibility: installation requiring only \texttt{pip install jax} (plus hardware-specific GPU support), simulation setup via readable YAML files, and execution on any JAX-supported platform without recompilation. This lowers entry barriers for plasma physics students, researchers from adjacent fields exploring kinetic turbulence, and small research groups without dedicated HPC resources. The code prioritizes time-to-first-simulation over absolute performance, complementing rather than replacing specialized HPC codes.

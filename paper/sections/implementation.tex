...
As reported in \S\ref{sec:numerics}, benchmarks on NVIDIA A100 GPUs achieve approximately $0.5$ seconds per time step for $256^3$ grids with $M=16$ moments. Memory usage scales as $N^3$ (approximately 460 MB for $256^3$) while computation scales as $N^3 \log N$, yielding super-linear cost increases with resolution. A $256^3$ turbulence simulation spanning 100 nonlinear eddy turnover times (with typical time steps $\Delta t \sim 0.01 t_{\mathrm{eddy}}$, where $t_{\mathrm{eddy}} = L_\perp/v_\perp$ characterizes perpendicular advection) requires $\sim$10,000 steps ($\sim$1.4 hours on A100), feasible for systematic parameter studies. JAX implementations can achieve performance competitive with specialized GPU codes for similar-scale problems \citep{Bauer2021,Citrin2024}, which we observe for GANDALF, while remaining portable across hardware platforms without modification.

On Apple Silicon (M1 Pro, M2 Max) using the Metal backend, empirical benchmarks show GANDALF delivers performance within a factor of 2--3 of A100 GPU results for grids up to $128^3$ (based on direct timing comparisons on both platforms), sufficient for code development, small parameter scans, and educational applications. This portability democratizes access to kinetic plasma turbulence simulations: researchers without GPU cluster access can perform meaningful science on laptop hardware, previously impossible with traditional HPC-focused codes.

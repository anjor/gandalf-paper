...
GANDALF advances the transformed variables using a midpoint (second-order Runge-Kutta) scheme. Integrating Eq.~\eqref{eq:integrating_factor_form} from $t_n$ to $t_{n+1/2}$ with constant right-hand side (the forward Euler approximation for the predictor step) gives
\begin{equation}
...
\end{equation}
Multiplying both sides by $e^{\pm i k_z \vA t_{n+1/2}}$ to obtain $\hat{w}^\pm_{n+1/2}$ yields
\begin{equation}
\hat{w}^\pm_{n+1/2} = e^{\pm i k_z \vA (t_{n+1/2} - t_n)}\left(\hat{w}^\pm_n + e^{\pm i k_z \vA (t_{n+1/2} - t_n)} \frac{\Delta t}{2} \widehat{\mathcal{N}_n^\pm}\right),
\end{equation}
producing the characteristic double phase factor structure. With $t_{n+1/2} - t_n = \Delta t/2$, the phase factors $\exp(\pm ik_z\vA\Delta t/2)$ appear \emph{twice}: once from removing the integrating factor at $t_{n+1/2}$ and once from integrating the nonlinear forcing from $t_n$. Given state $(\hat{w}^+_n, \hat{w}^-_n, \{\hat{g}^{\pm}_{m,n}\})$ at time $t_n$, we compute:
...
Typical performance on a $256^3$ grid with $M=16$ Hermite moments achieves $\sim 0.5$ seconds per time step on an NVIDIA A100 GPU, enabling multi-hundred-eddy-turnover turbulence simulations within days. JAX implementations can achieve performance competitive with compiled languages for scientific computing applications \citep{Bauer2021}, which we observe for GANDALF as well, while requiring substantially less development time through high-level array abstractions and automatic GPU compilation.

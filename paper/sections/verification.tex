...
Fully developed turbulence represents the most stringent test of a plasma turbulence code's ability to capture energy cascade dynamics. We simulate driven turbulence with Gaussian white noise forcing at large scales ($k \in [2, 5]$) to establish a steady-state inertial range with the characteristic Kolmogorov $k_\perp^{-5/3}$ spectrum predicted by critical balance theory \citep{GoldreichSridhar1995}.

\subsubsection{Physical Setup}

The simulation employs band-limited forcing that injects energy into Alfv\'en modes at the largest scales:
\begin{equation}
...
Steady state emerges when energy injection balances dissipation:
\begin{equation}
\varepsilon_{\text{inj}} = \int F \cdot \mathbf{u} \, d^3k = \varepsilon_{\text{diss}} = \eta \int k^2 |\mathbf{u}|^2 \, d^3k.
\end{equation}

We employ hyper-dissipation with $r=2$ and enhanced resistivity to maintain numerical stability while preserving an extended inertial range. For the $64^3$ resolution, anomalously high dissipation ($\eta = 20$) is required due to a known instability at this intermediate scale (see Appendix for discussion).
...
\end{equation}
where the sum over parallel wavenumbers captures the anisotropic nature of RMHD turbulence. Time averaging over the steady-state window (typically $t \in [30, 50]\tau_A$) provides robust statistics.
...
\begin{itemize}
\item \textbf{Inertial range scaling}: The spectrum exhibits $E(k_\perp) \propto k_\perp^{-\alpha}$ with $\alpha = 1.65 \pm 0.05$ over approximately one decade in wavenumber, consistent with the Kolmogorov prediction of $\alpha = 5/3 \approx 1.667$.
...
The measured spectral index of $\alpha = 1.65 \pm 0.05$ agrees with both theoretical predictions for strong Alfv\'enic turbulence \citep{GoldreichSridhar1995} and observational measurements in the solar wind \citep{Chen2013}. The slight deviation from exactly $5/3$ may reflect finite-size effects or the influence of kinetic damping at intermediate scales.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/turbulent_cascade_spectrum_fit.pdf}
\caption{Time-averaged turbulent energy spectrum $E(k_\perp)$ showing Kolmogorov scaling. Data averaged over steady state ($t \in [30, 50]\tau_A$) with error bars indicating temporal variance. Inertial range (shaded green) exhibits $k_\perp^{-1.65 \pm 0.05}$ scaling, consistent with theoretical $k_\perp^{-5/3}$ prediction (dashed line). Forcing range ($k \in [2, 5]$) and approximate dissipation scale marked. Resolution: $N = 64^3$ with anomalous dissipation $\eta = 20$.}

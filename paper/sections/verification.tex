...
\label{eq:spectrum}
\end{equation}
where $\elsphat$ and $\elsmhat$ are the Elsasser field Fourier coefficients (tilde denotes Fourier transform) and the sum accumulates all modes with perpendicular wavenumber magnitude in the bin centered at $k_\perp$. Time averaging over the quasi-steady-state window ($t \in [180, 200]\tau_A$) after initial transients decay provides robust statistics.

...
\item \textbf{Balanced forcing efficacy}: Restricting forcing to low $|k_z|$ modes ($|n_z| \leq 1$) prevents excess cross-helicity accumulation and maintains the balanced turbulence required for efficient cascade dynamics. Isotropic forcing in wavenumber space (equal power at all directions) can generate unbalanced states that suppress nonlinear energy transfer.

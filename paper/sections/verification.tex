...
We employ a single-$k$ forcing configuration that isolates the linear velocity-space cascade from perpendicular turbulent dynamics. The simulation forces only the $\kvec = (0, 0, 1)$ mode in the zeroth Hermite moment ($g_0$) of the $\elsp$ field, injecting energy at the coarsest velocity-space scale. With kinetic parameter $\lambda = -1.0$ (corresponding to $\alpha = 1.0$ in the original GANDALF thesis notation), the Elsasser field coupling ensures $z^\pm = 0$ identically, causing all perpendicular Poisson brackets to vanish. The dynamics reduce to purely linear Hermite coupling through the coefficients $\sqrt{m/2}$ and $\sqrt{(m+1)/2}$ in \Cref{eq:moment_hierarchy}, representing phase mixing without nonlinear cascade complications.

This configuration tests phase mixing in its cleanest form: particles with different parallel velocities $v_\parallel$ stream at different rates along the $z$-direction, causing initially coherent perturbations in $g_0$ to develop increasingly fine structure in velocity space. Energy cascades from $m=0$ to higher moments until balanced by collisional dissipation, establishing a steady-state spectrum that depends on the competition between phase mixing and velocity-space diffusion.

\subsubsection{Theoretical Expectation}

Linear KRMHD theory predicts a forward velocity-space cascade with power-law scaling. For weak collisionality where the collision frequency $\nu$ is small compared to the phase mixing rate, the Hermite moment spectrum follows:
\begin{equation}
C_m^\pm \sim m^{-1/2}
\label{eq:velocity_spectrum}
\end{equation}
in the inertial range $1 \ll m \ll m_{\text{diss}}$, where $m_{\text{diss}}$ marks the onset of collisional dissipation. This scaling arises from dimensional analysis: phase mixing transfers energy between adjacent moments at rate $\sim \omega m^{1/2}$ (from the coupling coefficients), while the collision operator with hyperdissipation $\nu m^{2n}$ provides scale-dependent damping. Balancing cascade and dissipation rates determines the dissipation scale, with the inertial range exhibiting the $m^{-1/2}$ spectrum characteristic of the forward Hermite cascade \citep{Schekochihin2009}.
